...
namespace ze {
    ...
    ZBuffer::~ZBuffer() {
        unmap();
        vkDestroyBuffer(zDevice.device(), buffer, nullptr);
        vkFreeMemory(zDevice.device(), memory, nullptr);
    }
    ...
    VkResult ZBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
        ...
        return vkMapMemory(zDevice.device(), memory, offset, size, 0, &mapped);
    }
    ...
    void ZBuffer::unmap() {
        if (mapped) {
            vkUnmapMemory(zDevice.device(), memory);
            mapped = nullptr;
        }
    }
    ...
    VkResult ZBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
        ...
        return vkFlushMappedMemoryRanges(zDevice.device(), 1, &mappedRange);
    }
    ...
    VkResult ZBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
        ...
        mappedRange.offset = offset;
        mappedRange.size = size;
        return vkInvalidateMappedMemoryRanges(zDevice.device(), 1, &mappedRange);
    }
     ...
     *
     */
    VkResult ZBuffer::flushIndex(int index) { return flush(alignmentSize, index * alignmentSize); }

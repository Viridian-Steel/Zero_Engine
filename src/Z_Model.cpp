...
namespace std {
	...
	struct hash<ze::ZModel::Vertex> {
		size_t operator()(ze::ZModel::Vertex const& vertex) const {
			size_t seed = 0;
...
namespace ze {
	ZModel::ZModel(ZDevice& device, const ZModel::Builder& builder): zDevice{ device }
	{
	...
	void ZModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		vertexBuffer = std::make_unique<ZBuffer>(
			zDevice,
			vertexSize,
			vertexCount,
		...
		zDevice.copyBuffer(stagingBuffer.getBuffer(), vertexBuffer->getBuffer(), bufferSize);
	...
	void ZModel::createIndexBuffers(const std::vector<uint32_t>& indices)
	{
		...
		indexBuffer = std::make_unique<ZBuffer>(
			zDevice,
			indexSize,
			indexCount,
		...
		zDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(), bufferSize);
	}

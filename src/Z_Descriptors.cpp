...
std::unique_ptr<ZDescriptorSetLayout> ZDescriptorSetLayout::Builder::build() const {
  return std::make_unique<ZDescriptorSetLayout>(zDevice, bindings);
}
...
ZDescriptorSetLayout::ZDescriptorSetLayout(
    ...
    : zDevice{zDevice}, bindings{bindings} {
...
ZDescriptorSetLayout::~ZDescriptorSetLayout() {
  vkDestroyDescriptorSetLayout(zDevice.device(), descriptorSetLayout, nullptr);
}
...
std::unique_ptr<ZDescriptorPool> ZDescriptorPool::Builder::build() const {
  return std::make_unique<ZDescriptorPool>(zDevice, maxSets, poolFlags, poolSizes);
}
...
ZDescriptorPool::ZDescriptorPool(
  ...
  if (vkCreateDescriptorPool(zDevice.device(), &descriptorPoolInfo, nullptr, &descriptorPool) !=
      VK_SUCCESS) {
...
ZDescriptorPool::~ZDescriptorPool() {
  vkDestroyDescriptorPool(zDevice.device(), descriptorPool, nullptr);
}
...
bool ZDescriptorPool::allocateDescriptor(
  ...
  if (vkAllocateDescriptorSets(zDevice.device(), &allocInfo, &descriptor) != VK_SUCCESS) {
    return false;
  }
  return true;
}
...
void ZDescriptorPool::resetPool() {
  vkResetDescriptorPool(zDevice.device(), descriptorPool, 0);
}
...
void ZDescriptorWriter::overwrite(VkDescriptorSet &set) {
  ...
  vkUpdateDescriptorSets(pool.zDevice.device(), writes.size(), writes.data(), 0, nullptr);
}
